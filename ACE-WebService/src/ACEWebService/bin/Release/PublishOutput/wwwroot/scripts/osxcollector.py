...
#  This work is a derivation of httpsgithub.comjipegitOSXAuditor
#
...
def _convert_to_local(func)
    ...
    source httpfeihonghsu.blogspot.com200802converting-from-local-time-to-utc.html
    '''
    def wrapper(args, kwargs)
        dt = func(args, kwargs)
...
ATTR_KMD_ITEM_WHERE_FROMS = 'com.apple.metadatakMDItemWhereFroms'
...
def _get_extended_attr(file_path, attr)
    ...
    Inspired by httpsgist.github.comdunhamsteve2889617
    ...
    try
        if isinstance(val, basestring)
            try
                ...
                # by protocol and port number, e.g. moc.elpmaxe.www.http80
        ...
        to_print = '[ERROR] _normalize_val {0}n'.format(repr(normalize_val_e))
...
class CodeSignChecker(object)
    ...
    def _load_framework(cls)
        ...
        if not cls.SEC_DLL
            cls.SEC_DLL = cls._load_library('SystemLibraryFrameworksSecurity.frameworkVersionsCurrentSecurity')
    ...
    def CFDictionary_objectForKey(cls, instance, key)
        Calls CFDictionaryobjectForKey
...
class Logger(object)
    ...
    Additionally, the Logger allows for extra keyvalue pairs to be set.  These will then
    ...
    def log_warning(cls, message)
        ...
        if DEBUG_MODE
            ...
            sys.stderr.write(' - {0}n'.format(repr(Logger.Extra.extras)))
...
class Collector(object)
    ...
    def _log_user_quarantines(self, homedir)
        ...
        Apple Support explanation of Quarantines httpsupport.apple.comkbHT3662
        ...
        # OS X = 10.7
        db_path = pathjoin(homedir.path, 'LibraryPreferencescom.apple.LaunchServices.QuarantineEventsV2')
        if not os.path.isfile(db_path)
            # OS X = 10.6
            db_path = pathjoin(homedir.path, 'LibraryPreferencescom.apple.LaunchServices.QuarantineEvents')
    ...
    def _log_xprotect(self)
        ...
        The plist for XProtect is at SystemLibraryCoreServicesCoreTypes.bundleContentsResourcesXProtect.plist

        XProtect also add minimum versions for Internet Plugins. That plist is at
        SystemLibraryCoreServicesCoreTypes.bundleContentsResourcesXProtect.meta.plist
        
        xprotect_files = [
            'SystemLibraryCoreServicesCoreTypes.bundleContentsResourcesXProtect.plist',
            'SystemLibraryCoreServicesCoreTypes.bundleContentsResourcesXProtect.meta.plist',
        ]
    ...
    def _log_startup_items(self, dir_path)
        ...
        httpsdeveloper.apple.comlibrarymacdocumentationmacosxconceptualbpsystemstartupchaptersStartupItems.html
    ...
    def _log_launch_agents(self, dir_path)
        ...
        httpsdeveloper.apple.comlibrarymacdocumentationDarwinReferenceManPagesman5launchd.plist.5.html
    ...
    def _log_user_launch_agents(self, homedir)
        path = pathjoin(homedir.path, 'LibraryLaunchAgents')
    ...
    def _log_user_login_items(self, homedir)
        ...
        plist_path = pathjoin(homedir.path, 'LibraryPreferencescom.apple.loginitems.plist')
        plist = self._read_plist(plist_path)
    ...
    def _collect_startup(self)
        ...
        # httpwww.malicious-streams.comarticleMac_OSX_Startup.pdf
        launch_agents = [
        ...
        packages = [
            'SystemLibraryScriptingAdditions',
            'LibraryScriptingAdditions',
        ]
    ...
    def _collect_downloads(self, homedir)
        ...
        directories_to_hash = [
            ('downloads', 'Downloads'),
            ('email_downloads', 'LibraryMail Downloads'),
            ('old_email_downloads', 'LibraryContainerscom.apple.mailDataLibraryMail Downloads')
        ]
    ...
    def _collect_firefox(self, homedir)
        ...
        all_profiles_path = pathjoin(homedir.path, 'LibraryApplication SupportFirefoxProfiles')
        if not os.path.isdir(all_profiles_path)
            Logger.log_warning('Directory not found {0}'.format(all_profiles_path))
            return

        # Most useful. See httpkb.mozillazine.orgProfile_folder_-_Firefox
    ...
    def _collect_safari(self, homedir)
        ...
        profile_path = pathjoin(homedir.path, 'LibrarySafari')
        ...
        plists = [
            ...
            ('extensions', 'ExtensionsExtensions.plist', 'Installed Extensions'),
        ]
    ...
    def _collect_chrome(self, homedir)
        ...
        chrome_path = pathjoin(homedir.path, 'LibraryApplication SupportGoogleChromeDefault')
    ...
    def _collect_accounts_system_admins(self)
        ...
        sys_admin_plist_path = pathjoin(ROOT_PATH, 'privatevardbdslocalnodesDefaultgroupsadmin.plist')
    ...
    def _collect_accounts_system_users(self)
        ...
        for user_name in listdir(pathjoin(ROOT_PATH, 'privatevardbdslocalnodesDefaultusers'))
            if user_name[0].startswith('.')
                continue

            user_details = {}

            sys_user_plist_path = pathjoin(ROOT_PATH, 'privatevardbdslocalnodesDefaultusers', user_name)
    ...
    def _collect_accounts_social_accounts(self, homedir)
        user_accounts_path = pathjoin(homedir.path, 'LibraryAccountsAccounts3.sqlite')
    ...
    def _collect_accounts_recent_items(self, homedir)
        ...
        recent_items_account_plist_path = pathjoin(homedir.path, 'LibraryPreferencescom.apple.recentitems.plist')
    ...
    def _collect_applications(self)
        ...
        with Logger.Extra('osxcollector_subsection', 'install_history')
            plist = self._read_plist(pathjoin(ROOT_PATH, 'LibraryReceiptsInstallHistory.plist'), default=[])
...
class kyphosis()
    ...
    def __init__(self, someFile, writeFile=False)
        ...
        self.supportedfiles = [xcaxfexbaxbe,  # FAT
                               xcfxfaxedxfe,  # x86
                               xcexfaxedxfe   # x86_64
                               ]
        # check if macho
...
def main()
    ...
    parser.add_argument(
        ...
        help='[OPTIONAL] Path to the OS X system to audit (e.g. mntxxx). The'
    ...
    if ROOT_PATH == '' and (euid != 0 and egid != 0)
        Logger.log_error('Must run as root!n')
        return
    ...
    sys.stderr.write('Wrote {0} lines.nOutput in {1}.tar.gzn'.format(Logger.lines_written, incident_id))
